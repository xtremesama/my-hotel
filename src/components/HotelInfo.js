...
const HotelInfo = () => {
    ...
    const loadServicesData = async () => {
        // Query API Gateway
        const response = await fetch("https://kee1b9u4r0.execute-api.us-east-1.amazonaws.com/Production/services");
    ...
    const loadAccessibilityData = async () => {
        // Query API Gateway
        const response = await fetch("https://kee1b9u4r0.execute-api.us-east-1.amazonaws.com/Production/accessibilities");
        let json = await response.json();
    ...
    useEffect(() => {
        // Load data from the API Gateway
        loadServicesData();
    }, [servicesData.name]);
        ...
        <div className="scene" id="hotelinfo">
            ...
            <article id="usefulinfo">
                ...
                <section className="checklist" id="services">
                    ...
                    <ul>
                        {
                            servicesData.map((item, index) => <li key={`services-${index}`}>{item.name}</li>)
                        }
                    </ul>
                </section>
                <section className="checklist" id="accessibility">
                    ...
                    <ul>
                        {
                            accessibilityData.map((item, index) => <li key={`accessibility-${index}`}>{item.name}</li>)
                        }
                    </ul>
                </section>
            </article>
